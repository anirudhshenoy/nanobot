...
def _make_provider(config: Config):
    """Create provider from config with direct/oauth and routed support."""
    ...
    if (
        ...
        and not (spec and (spec.is_oauth or spec.is_direct))
    ):
        console.print("[red]Error: No API key configured.[/red]")
        console.print("Set one in ~/.nanobot/config.json under providers section")
        raise typer.Exit(1)

    default_provider_name = provider_name or config.get_provider_name()
...
def gateway(
    ...
    agent = AgentLoop(
        ...
        memory_consolidation=config.agents.defaults.memory_consolidation,
